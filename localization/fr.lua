-- fr.lua
return {
    descriptions = {
        alt_texture = {
            alt_tex_quints_legendary_jokers = {
                name = "Jokers Légendaires",
                j_caino = {name = "Ichika"},
                j_yorick = {name = "Nino"},
                j_triboulet = {name = "Miku"},
                j_perkeo = {name = "Yotsuba"},
                j_chicot = {name = "Itsuki"},
            },

            alt_tex_quints_other_jokers = {
                name = "Autres Jokers",
                j_joker = {name = "Futaro"},
                j_diet_cola = {name = "Soda au Matcha"},
            },

            alt_tex_quints_spectrals = {
                name = "L'âme",
                c_soul = {
                    name = "L'âme des Nakano",
                },
            },
        },

        texture_packs = {
            texpack_quints_lumine_cards = {
                name = "Lumine's Quintuplets",
                text = {
                    "Remplace les Jokers légendaires et autres",
                    "par des personnages de",
                    "Go-Toubun no Hanayome!",
                    "{C:inactive,s:0.8}Art de Lumine et Turpix.{}",
                },
            },
        },
    },
}
